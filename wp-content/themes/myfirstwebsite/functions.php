...
<?php }
function myfirstwebsite_files()
{
    wp_enqueue_script('googleMap', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyBvq-mQ5q9HRXvfZcAIrqx1brXdUWzr4uI', NULL, '1.0', true);
    ...
    wp_localize_script('javascript', 'universityData', array(
        'root_url' =>get_site_url(),
        'nonce' => wp_create_nonce('wp_rest')
    ));
}
add_action('wp_enqueue_scripts', 'myfirstwebsite_files');
...
function university_adjust_queries($query)
{
    ...
    if (!is_admin() and is_post_type_archive('event') and $query->is_main_wuery()) {
        $today = date('Ymd');
...
function universityMapKey($api) {
    $api['key'] = 'AIzaSyBvq-mQ5q9HRXvfZcAIrqx1brXdUWzr4uI';
    return $api;
}
